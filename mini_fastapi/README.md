...
9. 기존 Router try/except 방식의 문제점

초기 구현에서는 Router 레이어에서 Service 호출을 try/except로 감싸고,
ValueError 등을 HTTPException으로 변환하는 방식을 사용했다.
...
결과적으로 Router가 “얇은 HTTP 어댑터” 역할을 수행하지 못하고
에러 정책의 중심이 되는 구조였다.

10. Global Exception Handler 구조 도입
...
이를 위해 app/core/exceptions.py에
예외 타입별 handler 함수를 정의하고, main.py에서 전역 등록했다.
...
Global Exception Handler에서
RequestValidationError를 명시적으로 처리함으로써:
...
Global Exception Handler 구조는 단순한 코드 정리가 아니라
운영 관점의 가시성을 개선한다.
